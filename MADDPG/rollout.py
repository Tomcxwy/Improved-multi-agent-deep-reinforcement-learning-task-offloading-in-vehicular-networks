                ...
                for i in range(self.device_num):
                    ...
                    for j in range(self.task_num + 1):
                        device_acts_[i].append((act[j * 10] + act[j * 10 + 1] + act[j * 10 + 2] + 
                                                ...
                                                act[j * 10 + 9]) / 20)
                    # 新增：将act转为可存储的格式（处理张量/数组）
                    # 如果act是torch张量，转numpy再转列表；如果是数值直接用
                    # print(device_acts_[i])
                    if isinstance(device_acts_[i], torch.Tensor):
                        act_val = device_acts_[i].cpu().detach().numpy().tolist()  # 张量转列表（兼容多维动作）
                    elif isinstance(device_acts_[i], np.ndarray):
                        act_val = device_acts_[i].tolist()
                    else:
                        act_val = device_acts_[i]  # 数值/列表直接用

                        # 核心修改：以追加模式写入总CSV文件
                    import csv
                    with open(self.all_acts_csv_path, 'a', newline='', encoding='utf-8') as f:
                        writer = csv.writer(f)
                        # 写入当前行数据（e_id, t_id, i, act_val）
