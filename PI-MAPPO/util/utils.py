...
class ObsScaling():
    def __init__(self):
        ...
        self.d_running_ms = RunningMeanStd(3 * params.device_num)
    ...
    def __call__(self, edge_obs, device_obss, evaluate):
        ...
        for obs in device_obss:
            nor_device_obss += [obs[0], obs[1], obs[2]]
        ...
        for i in range(params.device_num):
            device_obss[i][0] = float(nor_device_obss[i * 3])
            device_obss[i][1] = float(nor_device_obss[i * 3 + 1])
            device_obss[i][2] = float(nor_device_obss[i * 3 + 2])

            for j in range(params.task_num):
                device_obss[i][3 + j * 3] /= self.max_data_size
                device_obss[i][3 + j * 3 + 1] /= self.max_comp_dens
                device_obss[i][3 + j * 3 + 2] /= self.max_dly_cons
